...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for theme840
 */

// Auto-rebuild the theme registry during theme development.
if (theme_get_setting('theme840_clear_registry')) {
  // Rebuild .info data.
 ...
 */
function theme840_html_head_alter(&$head_elements) {
 ...
 */
function theme840_preprocess_search_block_form(&$vars) {
 ...
 */
function theme840_preprocess(&$vars, $hook) {
  $vars['theme840_path'] = base_path() . path_to_theme();
}

/**
 * Implements template_preprocess_html().
 */
function theme840_preprocess_html(&$vars) {
  
  $vars['doctype'] = _theme840_doctype();
  $vars['rdf'] = _theme840_rdf($vars);
  ...
  if (!$vars['is_front']) {
    ...
    if (isset($page_name)) {
      $vars['classes_array'][] = theme840_id_safe('page-' . $page_name);
    }

    $vars['classes_array'][] = theme840_id_safe('section-' . $section);
 ...
 */
function theme840_preprocess_page(&$vars) {
  if (arg(0) == 'blog') {
    drupal_set_title("專欄文章");
  }
 ...
 */
function theme840_preprocess_maintenance_page(&$vars) {
  ...
  drupal_add_css(drupal_get_path('theme', 'theme840') . '/css/maintenance-page.css');
}
 ...
 */
function theme840_preprocess_node(&$vars) {
  // Striping class
  ...
  $vars['submitted'] = t('!username 發表於', array('!username' => $vars['name']));
 ...
 */
function theme840_preprocess_block(&$vars, $hook) {
 ...
 */
function theme840_menu_tree($vars) {
 ...
 */
function theme840_field__taxonomy_term_reference($vars) {
  $output = '';
  ...
  $output = '<div class="' . $vars['classes'] . '">' . $output . '</div>';
 ...
 */
function theme840_breadcrumb($vars) {
  
  $breadcrumb = isset($vars['breadcrumb']) ? $vars['breadcrumb'] : array();

// Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('theme840_breadcrumb_display');
  if ($show_breadcrumb == 'yes') {
  
	  if (theme_get_setting('theme840_breadcrumb_hideonlyfront')) {
		$condition = count($breadcrumb) > 1;
	  } else {
		$condition = !empty($breadcrumb);
	  }
	  
	  if(theme_get_setting('theme840_breadcrumb_showtitle')) {
	  ...
	  $separator = theme_get_setting('theme840_breadcrumb_separator');
 ...
 * @return bool
 */
function theme840_tabs_float() {
  $float = (bool) theme_get_setting('theme840_tabs_float');
  $float_node = (bool) theme_get_setting('theme840_tabs_node');
...
function theme840_id_safe($string) {
  // Strip accents
 ...
 * @ingroup themeable
 */
function theme840_menu_link(array $vars) {
  ...
  $element['#attributes']['class'][] = theme840_id_safe($element['#title']);
 ...
 */
function theme840_preprocess_menu_local_task(&$vars) {
 ...
 */
function theme840_menu_local_tasks(&$vars) {
  $output = '';
...
function theme840_theme(&$existing, $type, $theme, $path) {
